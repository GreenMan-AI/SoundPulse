...
// GET /api/tracks/:id/comments — visi komentāri (publiski)
...
// POST /api/tracks/:id/comments — pievieno komentāru (jābūt ielogotam)
app.post('/api/tracks/:id/comments', requireAuth, async (req, res) => {
  try {
    if (!isValidId(req.params.id)) return badId(res);
    const { text } = req.body || {};
    if (!text?.trim()) return res.status(400).json({ error: 'Teksts ir obligāts' });
    if (text.trim().length > 300) return res.status(400).json({ error: 'Pārāk garš (max 300 rakstzīmes)' });

    const track = await Track.findById(req.params.id);
    if (!track) return res.status(404).json({ error: 'Dziesma nav atrasta' });
    ...
    // Atjauno komentāru skaitu dziesmā
    await Track.findByIdAndUpdate(req.params.id, { $inc: { commentCount: 1 } });

    res.status(201).json({ comment });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// DELETE /api/comments/:id — dzēš komentāru (TIKAI ADMIN)
app.delete('/api/comments/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    if (!isValidId(req.params.id)) return badId(res);
    const comment = await Comment.findByIdAndDelete(req.params.id);
    if (!comment) return res.status(404).json({ error: 'Komentārs nav atrasts' });

    // Samazina komentāru skaitu dziesmā
...
// GET /api/comments/all — visi komentāri (tikai admin — moderācijai)
app.get('/api/comments/all', requireAuth, requireAdmin, async (req, res) => {
  try {
    const comments = await Comment
      .find()
      .sort({ createdAt: -1 })
...
app.post('/api/upload', requireAuth, uploadLimiter, (req, res) => {
  uploadAudio.single('audio')(req, res, async (err) => {
    ...
    try {
      if (!req.file) return res.status(400).json({ error: 'Nav faila' });
      // ── Dienas limits pārbaude ──
      const _used = _getUploadCount(req.user.username);
      if (_used >= UPLOAD_MAX_DAY) return res.status(429).json({ error: `Dienas limits sasniegts — max ${UPLOAD_MAX_DAY} augšupielādes/dienā` });
...
app.get('/api/upload/limits', requireAuth, (req, res) => {
  ...
  res.json({ maxSizeMB: UPLOAD_MAX_MB, maxDurationMin: 6, uploadsPerDay: UPLOAD_MAX_DAY, limit: UPLOAD_MAX_DAY, usedToday: used, used, remaining: Math.max(0, UPLOAD_MAX_DAY-used), canUpload: used < UPLOAD_MAX_DAY, allowedTypes: ['.mp3','.wav','.ogg','.flac','.m4a','.aac'] });
});
...
app.get('/manifest.json', (req, res) => {
  res.json({
    name: 'SoundPulse',
    short_name: 'SoundPulse',
    description: 'SoundPulse — Latvijas mūzikas platforma',
    start_url: '/',
    display: 'standalone',
